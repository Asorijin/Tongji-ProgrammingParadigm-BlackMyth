...
AToolMp::AToolMp()
{
    ...
    // 绑定重叠事件（必须在构造函数中绑定！）
    InteractionTrigger->OnComponentBeginOverlap.AddUniqueDynamic(this, &AToolMp::OnOverlapBegin);
    InteractionTrigger->OnComponentEndOverlap.AddUniqueDynamic(this, &AToolMp::OnOverlapEnd);
...
// Called every frame
void AToolMp::Tick(float DeltaTime)
{
    ATools::Tick(DeltaTime);
...
void AToolMp::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
...
void AToolMp::OnOverlapEnd(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
